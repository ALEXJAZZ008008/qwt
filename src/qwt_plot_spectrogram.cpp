...
public:
    ...
    {
        colorMap.reset(new QwtLinearColorMap());
...
void QwtPlotSpectrogram::setColorMap( QwtColorMap *colorMap )
{
    ...
    {
        d_data->colorMap.clear();
        d_data->colorMap.reset(colorMap);
    }
...
void QwtPlotSpectrogram::setColorMap(const QSharedPointer<QwtColorMap> colorMap )
{
    if ( colorMap == NULL )
        return;
    ...
    const QRect &tile, QImage *image ) const
{
    ...
    if ( d_data->colorMap->format() == QwtColorMap::RGB )
    {
        ...
        const QwtColorMap *colorMap = d_data->colorMap.get();
